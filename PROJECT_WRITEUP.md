...
Every arm, the random control included, was judged against the same unsteered baseline, so a method clears the control when its win rate over that baseline sits significantly above the control's. Twenty-five of seventy-two model-axis-method conditions clear it on Mistral and twenty-six on Prometheus. Only four and five respectively clear the unsteered baseline itself.

The two judges agree on the size of the effect and disagree on where it falls. Both place religion at the bottom, at three conditions on Mistral and one on Prometheus out of twenty-four. They split on the other two axes, with Mistral seeing eleven on each of origin and age and Prometheus seeing sixteen and nine. We report the disagreement rather than picking the judge that suits us.

The gap between those two counts matters, and we think it is the most easily misread number in this work. Beating the random control means the pluralism direction moves the output more than noise of the same magnitude does. It does not mean the answer improved. Twenty-one of the significant conditions still fail to clear the unsteered baseline on either judge, and in those cases the vector is degrading the answer less than noise would rather than improving it. The median decisive win rate across the significant conditions is 0.406 on Mistral and 0.468 on Prometheus, so the baseline is preferred more often than not.
...
The three bands never overlap. Origin's largest value sits below age's smallest, and age's largest sits below religion's smallest. This holds across two families and a 23-fold spread in hidden-state norm. The raw vector norms vary 78-fold across the eighteen cells, which we had initially taken to be idiosyncratic; it is not, because hidden-state norms vary alongside them and the ratios stay in place. *Steering vector magnitude scales with representation scale.*
...
We used an injection depth of 45%, taken from Im and Li's result on a 7B model. A sweep across five depths shows that the two families disagree. All three SmolLM2 models peak at the shallowest depth we tested, 25%, and on every one of those rows the inherited 45% falls between the two worst points available. None of the Qwen models peak there: Qwen2.5-3B peaks at 85%, Qwen2.5-0.5B at 70% and Qwen2.5-1.5B at 55%. Qwen2.5-1.5B deserves separate mention because it is negative at every depth, so its peak is the least bad option rather than a good one and we do not read it as evidence for a depth.
...
**Judge agreement is better than kappa suggests.** Our two judges reach a pooled Cohen's κ of 0.329, falling to 0.248 averaged over the eighteen model-axis cells, which reads as only fair either way. But across 17,950 shared pairs they agree outright on 61.4% and name opposite winners on 373, or 2.08%. Almost all disagreement concerns where the tie line falls, and κ penalises a tie-against-win mismatch exactly as harshly as a full reversal. We report both figures.
